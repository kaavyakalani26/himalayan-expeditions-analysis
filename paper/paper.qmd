...
For my analysis, a logistic regression model will be used to model if the person will be successful on their attempt to summit the particular Himalayan peak. The model will be based on five independent demographic variables: `height of the peak`, `sex`, `age`, `seasons` and `solo` and the dependent variable will be `success`.
